...
def Open_File():
    os.startfile('C:\\Users\\Fawad computer\\Desktop\\Python\\CustomerRecords.txt')

def records_window_quit():
    global records_window
    records_window.destroy()

def Delete_Records():
    Confirm_Delete = tmsg.askokcancel("Sure?", "Deleting All Your Records Might Be Risky Do You Want To Do It?" )
    if Confirm_Delete == True:
        os.remove('C:\\Users\\Fawad computer\\Desktop\\Python\\CustomerRecords.txt')
    

def Searching():
    global Entry_Find , listbox_2
    search_terms = Entry_Find.get().lower()

    listbox_2.selection_clear(0 , END)

    for i in range(listbox_2.size()):
        item = listbox_2.get(i).lower()
...
def Show_Records():
    global Entry_Find , listbox_2 , records_window
    print('showing records')

    records_window = Toplevel(root)
    records_window.title("Records")
    records_window.geometry("700x400")
    records_window.maxsize(700,400)
    records_window.minsize(700,400)



    Find_Label = Label(records_window, text="Find :")
    Find_Label.pack(side=LEFT , anchor = "nw")

    Entry_Find = Entry(records_window)
    Entry_Find.pack(side=LEFT , anchor = "nw")

    search_button = Button(records_window, text="Search",command=Searching)
    search_button.pack(side=LEFT , anchor = "nw") 

    scrollbar = Scrollbar(records_window)
    scrollbar.pack(side=RIGHT , fill=Y)

    listbox_2 = Listbox(records_window, width=60, height=25 , yscrollcommand = scrollbar.set)  # Increased size
    listbox_2.pack(pady=10, padx=10, fill=BOTH, expand=True)  # Added fill/expand

    scrollbar.config(command=listbox_2.yview)

    Menu_Options = Menu(records_window)
    MenuSubmenu_Options = Menu(Menu_Options , tearoff=0)
    MenuSubmenu_Options.add_command(label="Close" , command=records_window_quit)
    MenuSubmenu_Options.add_command(label="Write/Remove" , command=Open_File)
    MenuSubmenu_Options.add_separator()
    MenuSubmenu_Options.add_command(label="Delete Records" , command=Delete_Records)
    records_window.config(menu=Menu_Options)

    Menu_Options.add_cascade(label="Options", menu=MenuSubmenu_Options)


    try:
        with open("CustomerRecords.txt" , "r") as f:
            for line in f:
                listbox_2.insert(END, line.strip())    
    except FileNotFoundError:
        listbox_2.insert(END , "NO RECORDS")
...
def Submit():
    ...
    if paid < 0 and price < 0:
        tmsg.showerror("Invalid Input", "Amounts Can Not Be Negative. Please enter valid values.")
...
# TODO: Creating Designs - Converted coordinates from 400x400 to 600x642 ratio
Canvas_Creation.create_rectangle(600,0,525,77, fill = "#32CD32", width=2)
Canvas_Creation.create_rectangle(0,0,75,77, fill="#32CD32", width=2)
Canvas_Creation.create_line(75,0,15,15, width=2)
Canvas_Creation.create_line(525,0,585,15,width=2)

# TODO: Creating Designs For Lower Rectangle
Canvas_Creation.create_rectangle(0,97,20,130, fill = "#32CD32", width =2)
Canvas_Creation.create_rectangle(600,97,580,130,fill = "#32CD32" , width=2)

Canvas_Creation.create_rectangle(15,15,585,92, fill = "#32CD32", width =2)
Canvas_Creation.create_text(300,54, text ="Customer Service" , font ="Calibri 23 bold" , fill ="black")

Canvas_Creation.create_rectangle(15,98,585,138, fill="#32CD32",width=2)
Canvas_Creation.create_text(300,118, text="Daily Submissions Forms Records")

# Names Entries
Canvas_Creation.create_text(180,195, text="Product :" , font="Aldhabi 9 bold")
Canvas_Entry_Names_var = StringVar()
Canvas_Entry_Names = Entry(Canvas_Creation, textvariable=Canvas_Entry_Names_var , borderwidth = 2, font="Verdana")
Window_Entry = Canvas_Creation.create_window( 330,195, window=Canvas_Entry_Names)

# Price Entry 
Canvas_Creation.create_text(180,233, text=" Price :" , font="Aldhabi 9 bold")
Canvas_Entry_Price_var = StringVar()
Canvas_Entry_Price = Entry(Canvas_Creation, textvariable=Canvas_Entry_Price_var, borderwidth = 2, font="Verdana")
Window_Entry = Canvas_Creation.create_window( 330,233, window=Canvas_Entry_Price)

# Paid Amount
Canvas_Creation.create_text(170,270, text="Amount Paid :" , font="Aldhabi 9 bold")
Canvas_Entry_Paid_var = StringVar()
Canvas_Entry_Paid = Entry(Canvas_Creation, textvariable=Canvas_Entry_Paid_var, borderwidth = 2 , font="Verdana")
Window_Entry = Canvas_Creation.create_window( 330,270, window=Canvas_Entry_Paid)

# Button Submit
Button_Submit = Button(root , text=" Submit ", command=Submit , highlightthickness=2 , relief=GROOVE , padx=20)
Window_Button = Canvas_Creation.create_window(330, 323, window=Button_Submit)
...
Menu_Submenu.add_command(label="Show Records" , command=Show_Records)
Menu_Bar.add_cascade(label="Options", menu=Menu_Submenu)

# Lower Note Rectangle :
Canvas_Creation.create_rectangle(15,375,585,435, width=2)
Canvas_Creation.create_text(300,390, text="Dear Users Your Records will be Saved within your Directory with File")
Canvas_Creation.create_text(300,410, text="Named (CustomerRecords.txt) and can be accessed anytime !")
